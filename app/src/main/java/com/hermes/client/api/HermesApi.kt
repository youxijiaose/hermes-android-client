...
class HermesApi(private val baseUrl: String) {
    ...
    suspend fun getSessions(): Result<List<SessionInfo>> {
        return withContext(Dispatchers.IO) {
            try {
                val req = Request.Builder().url("$baseUrl/api/sessions").build()
                val response = restClient.newCall(req).execute()
    ...
    suspend fun createSession(): Result<String> {
        return try {
            val result = gateway.request("session.create", mapOf("source" to "android"))
    ...
    suspend fun resumeSession(sessionId: String): Result<List<MessageWrapper>> {
        return try {
            val result = gateway.request("session.resume", mapOf("session_id" to sessionId))
    ...
    suspend fun deleteSession(sessionId: String): Result<Boolean> {
        return try {
            val result = gateway.request("session.delete", mapOf("session_id" to sessionId))
    ...
    suspend fun interruptSession(sessionId: String): Result<Boolean> {
        return try {
            val result = gateway.request("session.interrupt", mapOf("session_id" to sessionId))
            Result.success(result != null)
        } catch (e: Exception) {
            Result.failure(e)
        }
